...
export default function PricingPage({ globals = {}, overrides = [], orderbook = [], onChanged }) {
  ...
  const [overrideForm, setOverrideForm] = useState({ upstream: '', model_id: '', trigger_pct: '', rebound_pct: '' });
  ...
  const saveGlobal = async upstream => {
    ...
    try {
      const response = await apiFetch('/api/pricing/global', {
        ...
        body: JSON.stringify({ upstream, ...fieldNames.reduce((out, field) => ({ ...out, [field]: numberValue(cfg[field]) }), {}) }),
      });
  ...
  const saveOverride = async event => {
    ...
    const { upstream, model_id, trigger_pct, rebound_pct } = overrideForm;
    ...
    try {
      const response = await apiFetch('/api/auto-pricing/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey() },
        body: JSON.stringify({ upstream, model_id, trigger_pct: Number(trigger_pct), rebound_pct: rebound_pct === '' ? null : Number(rebound_pct) }),
      });
      const data = await response.json();
      ...
      setOverrideForm({ upstream: '', model_id: '', trigger_pct: '', rebound_pct: '' });
      onChanged?.();
    } catch (error) { setMessage(`Error: ${error.message}`); } finally { setBusy(null); }
  };
  ...
  return (
    <div className="page pricing-page" aria-label="Pricing control plane">
      ...
      <section className="panel pricing-section">
        <div className="panel-head"><div><h3>Global per Upstream</h3><div className="sub">Caps and revenue shares applied by upstream.</div></div></div>
      ...
      <section className="panel pricing-section">
        <div className="panel-head"><div><h3>Per-Model Override</h3><div className="sub">Override trigger and rebound bands for a specific upstream/model pair.</div></div></div>
        <form className="pricing-override-form" onSubmit={saveOverride}>
          {['upstream', 'model_id', 'trigger_pct', 'rebound_pct'].map(field => <label className="pricing-field" key={field}><span>{field}</span><input type={field.endsWith('_pct') ? 'number' : 'text'} step={field.endsWith('_pct') ? '0.0001' : undefined} value={overrideForm[field]} onChange={event => setOverrideForm(prev => ({ ...prev, [field]: event.target.value }))} placeholder={field === 'rebound_pct' ? 'optional' : ''} /></label>)}
        ...
        <div className="table-scroll"><table className="tbl"><thead><tr><th>Upstream / model</th><th>trigger_pct</th><th>rebound_pct</th><th>updated_at</th><th>Aksi</th></tr></thead><tbody>
          {overrideRows.map(override => { const key = actionKey('delete', override.id); return <tr key={`${override.upstream}-${override.model_id}`}><td>{override.upstream} / {override.model_id}</td><td className="tnum">{formatValue(override.trigger_pct)}</td><td className="tnum">{formatValue(override.rebound_pct)}</td><td className="faint">{override.updated_at || '—'}</td><td><button className="btn btn-sm btn-danger" onClick={() => deleteOverride(override)} disabled={!override.id || busy === key}>Hapus</button></td></tr>; })}
          {!overrideRows.length && <tr><td colSpan={5} className="dt-empty">Belum ada override tersimpan.</td></tr>}
        </tbody></table></div>
      </section>

      <section className="panel pricing-section">
        <div className="panel-head"><div><h3>Orderbook · merged asks + auto-pricing</h3><div className="sub">Our asks are marked separately from competitor levels.</div></div></div>
        <div className="table-scroll"><table className="tbl"><thead><tr><th>Model</th><th>Min ask</th><th>Max ask</th><th>Spread</th><th>Our ask</th><th>Levels</th></tr></thead><tbody>
          {orderbookRows.map(row => <tr key={row.model_id}><td><strong>{row.label || row.model_id}</strong><div className="prov-sub">{row.model_id}</div></td><td className="tnum">{formatValue(row.min_ask ?? row.ask)}</td><td className="tnum">{formatValue(row.max_ask)}</td><td className="tnum">{formatValue(row.spread)}</td><td className="tnum pos">{formatValue(row.our_ask)}</td><td><div className="pricing-levels">{(row.upstreams || []).flatMap(upstream => (upstream.levels || []).map((level, index) => <span className={upstream.is_ours ? 'pricing-level ours' : 'pricing-level'} key={`${upstream.slug}-${index}`}>{formatValue(level.price)} · {upstream.is_ours ? 'ours' : upstream.label || upstream.slug}</span>))}{!row.upstreams?.length && row.ask != null && <span className="pricing-level">ask: {formatValue(row.ask)}</span>}</div></td></tr>)}
          {!orderbookRows.length && <tr><td colSpan={6} className="dt-empty">Belum ada data orderbook.</td></tr>}
        </tbody></table></div>
      </section>
    </div>
  );
